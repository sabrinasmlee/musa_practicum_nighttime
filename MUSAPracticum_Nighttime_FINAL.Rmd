...
```{r unnesting hour dataset, echo=TRUE}
fishnet <- 
...
do.call(grid.arrange, c(mapList, 
                        ncol=1, top="Count of Nightlife Businesses per Fishnet", 
                        bottom = "Figure 3.1.2"))
```
...
Now that we have investigated the distribution of nightlife establishments across Philadelphia, it is important to understand how these establishments have been impacted by the COVID-19 pandemic. Using SafeGraph data, we are able to see that the pandemic has a drastic impact in foot traffic across sectors and space, though not necessarily evenly. First, we observe citywide changes in foot traffic over the course of the pandemic in the below Figure 3.2.1. Comparing 2020 traffic to our 2018 dataset, we see that traffic across the city drastically decreased in March 2020 and represented less then half of the 2018 traffic. This trend continued throughout the year.
...
Figures 3.4.1 below shows the volume of visitors to the selected corridors by census block group. [While visitors from across the city frequent East Girard corridor, particularly for its restaurants, Figure X.X clarifies that the corridor mostly caters to a local customer base of surrounding census tracts.] Block groups shaded grey indicate that no resident made a trip to the corridor.

```{r CBG data wrangling, echo=TRUE}
corr_select <-
  phl_corridors %>%
...
```{r flows data wrangling, echo=TRUE}
flows <- dat2 %>%
  filter(top_category == "Drinking Places (Alcoholic Beverages)" |
           top_category == "Restaurants and Other Eating Places" |
           top_category == "Promoters of Performing Arts, Sports, and Similar Events" |
...
```{r loading in prediction data, echo=TRUE, message=FALSE, warning=FALSE}
#SafeGraph Features
# bar.sf <- dat2 %>%
#   filter(top_category == "Drinking Places (Alcoholic Beverages)") %>%
...
count_bars_a_log | SafeGraph | SafeGraph | Log transformation of bar density variable
count_rest_a_log | SafeGraph | SafeGraph | Log transformation of restaurant density variable
count_arts_a_log | SafeGraph | SafeGraph | Log transformation of arts venue density variable
count_grocery_a_log | SafeGraph | SafeGraph | Log transformation of grocery density variable
count_sports_a | SafeGraph | SafeGraph | Log transformation of sports establishment density variable
...
count_openlate_log | SafeGraph | SafeGraph | Log transformation of businesses open after 8PM based on open_hours variable
...
count_late_tag | SafeGraph | SafeGraph | Count of businesses with "late" tag
corr_type | Built Environment | [x] | Corridor typology (see detailed description above)
count_late_log | SafeGraph | SafeGraph | Log transformation of count of businesses with "late" tag
...
map_preds_rf %>%
  ...
  st_drop_geometry() %>%
  dplyr::select(date_range_start, MAE, MAPE) %>%
  # mutate(date_range_start = as.numeric(as.date(date_range_start))) %>%
  mutate(date_range_start = str_remove_all(date_range_start, pattern = "T05:00:00Z"),
         date_range_start = str_remove_all(date_range_start, pattern = "T04:00:00Z")) %>%
...
map_preds_corr_rf[order(factor(map_preds_corr_rf$corr_type,
                            levels = c("Neighborhood Subcenter",
                                       ...
                                       "Superregional Center",
                                       "Specialty Center"))),] %>%
  kable(caption = "Random Forest Model: Errors by Corridor Type") %>%
  kable_styling()
```

### Mapping Errors

As a final evaluation of the random forest model, we map the errors across Philadelphia commercial corridors in the below Figures X.X and X.X. The map on the left shows the highest quintile of absolute error tends to occur in smaller corridors. There is a particular cluster in far West Philadelphia and the area north of Center City. When reviewing the Percent Error metric on the right, the best predictions are concentrated in Center City, likely because these corridors see more traffic overall.
```{r echo=TRUE}
map_preds_space <-
  map_preds_rf %>%
