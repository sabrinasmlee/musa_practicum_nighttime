...
scenario1 <-
  dat_pred_agg %>%
  mutate(count_rest_a = count_rest_a*1.25, #Confirm that this operation works
         count_rest_a_log = log(count_rest_a))
scenario1.preds <- predict(rf1, data = st_drop_geometry(scenario1))
scenario1.preds <- exp(scenario1.preds$predictions)

#Scenario 2 - Decrease of restaurants by 25%
scenario2 <-
  dat_pred_agg %>%
  mutate(count_rest_a = count_rest_a*.75,
         count_rest_a_log = log(count_rest_a))
...
scenario3 <-
  dat_pred_agg %>%
  mutate(count_bars_a = count_bars_a*1.25,
         count_bars_a_log = log(count_bars_a))
