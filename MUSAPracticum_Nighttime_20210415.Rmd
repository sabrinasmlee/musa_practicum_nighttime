...
#Scenario 1 - Increase of restaurants by 25%
scenario1 <-
  dat_scenario %>%
  mutate(restaurants = restaurants * 1.1 , 
         count_rest_a = restaurants/phl_area_sqmi, #Confirm that this operation works
         count_rest_a_log = log(count_rest_a))
scenario1.preds <- predict(rf1, data = scenario1)
scenario1.preds <- exp(scenario1.preds$predictions)

#Scenario 2 - Decrease of restaurants by 25%
scenario2 <-
  dat_scenario %>%
  mutate(restaurants = restaurants *.9,
         count_rest_a = restaurants/phl_area_sqmi,
         count_rest_a_log = log(count_rest_a))
scenario2.preds <- predict(rf1, data = st_drop_geometry(scenario2))
scenario2.preds <- exp(scenario2.preds$predictions)

#Scenario 3 - Increase of bars by 25%
scenario3 <-
  dat_scenario %>%
  mutate(bars = bars *1.1,
         count_bars_a = bars/phl_area_sqmi,
         count_bars_a_log = log(count_bars_a))
scenario3.preds <- predict(rf1, data = st_drop_geometry(scenario3))
scenario3.preds <- exp(scenario3.preds$predictions)

#Scenario 2 - Decrease of bars by 25%
scenario4 <-
  dat_scenario %>%
  mutate(count_bars_a = count_bars_a*.75,
         count_bars_a_log = log(count_bars_a))
scenario4.preds <- predict(rf1, data = st_drop_geometry(scenario4))
...
ggplot(Scenario_preds, aes(x =Night_visits)) +
  geom_histogram()
