...
dat_pred_agg %>%
  st_drop_geometry() %>%
  pivot_longer(., cols = c(starts_with("count_retail")), names_to = "variable", values_to="value") %>%
  ggplot(aes(value, Night_visits_sqmi_log)) +
  geom_point(size = .5) +
  geom_smooth(method = "lm", se=F, colour = "#FA7800") +
...
With the following code, it seems clear that Market East is the problem corridor, one of three  Supperregional Centers. **Should we exclude Market East from the model?**
```{r}
preds.corr.train <- data.frame(corridor = phl.training$corridor,
           dat_range_start = phl.training$date_range_start,
           pred   = exp(predict(reg1)),
           actual = phl.training$Night_visits_sqmi,
           source = "training data")
preds.corr.test  <- data.frame(corridor = phl.test$corridor,
           dat_range_start = phl.test$date_range_start,
           pred   = reg1_predict,
           actual = phl.test$Night_visits_sqmi,
           source = "testing data")

rbind(preds.corr.test, preds.corr.train) %>%
  filter(pred > 400000) %>%
  kable(caption = "Which corridors have the outlier predictions?") %>% kable_styling()

dat_pred_agg %>% filter(Night_visits_sqmi > 600000) %>%
  select(corridor) %>% st_drop_geometry() %>%
  kable(caption = "Which corridors have the highest visits per square mile?") %>%
  kable_styling()

# test <- dat_pred_agg %>% select(corridor, phl_area_sqmi) %>% st_drop_geometry() %>%
#   group_by(corridor) %>%
#   summarize(phl_area_sqmi = mean(phl_area_sqmi))
```
...
```{r removing outliers, out.width = "100%"}
dat_pred_agg %>%
...
#having trouble with the reordering here.
...
set.seed(414)
inTrain <- createDataPartition(y=dat_pred_agg_sub_rf$Night_visits_sqmi_log, p = .60, list = FALSE)

phl.training.rf.sub <- dat_pred_agg_sub_rf[inTrain,]
phl.test.rf.sub <- dat_pred_agg_sub_rf[-inTrain,]
