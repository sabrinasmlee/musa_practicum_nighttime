...
phl_corridors_pred <- phl_corridors %>%
  ...
  rename(., corridor = NAME,
         district = P_DIST,
         vacancy = VAC_RATE) %>%
  mutate(corr_area_sqft = as.numeric(st_area(phl_corridors)),
...
dat_pred_agg <- 
  ...
  summarize(Night_visits = sum(Hrs19_0),
            ...
            Night_visits_sqmi = Night_visits / phl_area_sqmi,
            Night_visits_sqmi_log = log(Night_visits_sqmi +1),
            total = sum(total),
            phl_building_area = mean(bld_area_sqft, na.rm = TRUE),
            phl_building_area_log = log(phl_building_area +1),
            ...
            count_bars_a = sum(bars, na.rm = TRUE)/phl_area_sqmi,
            count_rest_a = sum(restaurant, na.rm = TRUE)/phl_area_sqmi,
            count_arts_a = sum(arts, na.rm = TRUE)/phl_area_sqmi,
            # count_jrcol_a = sum(jrcol, na.rm = TRUE)/phl_area_sqmi,
            count_college_a = sum(college, na.rm = TRUE)/phl_area_sqmi,
            count_sports_a = sum(sports, na.rm = TRUE)/phl_area_sqmi,
            count_museums_a = sum(museum, na.rm = TRUE)/phl_area_sqmi,
            count_amuse_a = sum(amusement, na.rm = TRUE)/phl_area_sqmi,
            count_hotels_a = sum(hotels, na.rm = TRUE)/phl_area_sqmi,
            count_late_tag = sum(late_night, na.rm = TRUE)/phl_area_sqmi,
            count_barpub_tag = sum(bar_pub, na.rm = TRUE)/phl_area_sqmi,
            count_drinks_tag = sum(drinks, na.rm = TRUE)/phl_area_sqmi,
            count_cocktail_tag = sum(cocktail, na.rm = TRUE)/phl_area_sqmi,
            count_bars_a_log = log(count_bars_a +1),
            count_rest_a_log = log(count_rest_a +1),
            # log_arts_a = log(sum(arts, na.rm = TRUE)/phl_area_sqmi),
            ...
            count_amuse_a_log = log(count_amuse_a +1),
            ...
            count_barpub_log = log(count_barpub_tag +1),
            count_late_log = log(count_late_tag +1),
            ...
            demo_pctHisp_log = log(demo_pctBlack +1)) %>% 
  mutate_if(is.numeric, list(~na_if(., Inf))) %>%
  st_drop_geometry() %>%
...
dat_pred_agg %>%
  ...
  mutate(phl_area_sqmi_log = log(phl_area_sqmi),
         phl_building_area_log = log(phl_building_area)) %>%
  pivot_longer(., cols = starts_with("phl_"), names_to = "variable", values_to="value") %>%
  ggplot(aes(value)) +
  geom_histogram() +
...
dat_pred_agg %>%
  st_drop_geometry() %>%
  select(sg_distance_home) %>%
  mutate(sg_distance_home_log = log(sg_distance_home)) %>%
  pivot_longer(., cols = starts_with("sg_"), names_to = "variable", values_to="value") %>%
  ggplot(aes(value)) +
  geom_histogram() +
...
dat_pred_agg %>%
  ...
  # mutate(nn_CenterCity_log = log(nn_CenterCity),
  #        nn_parks_log = log(nn_parks),
  #        nn_transit_log = log(nn_transit)) %>%
  pivot_longer(., cols = starts_with("nn_"), names_to = ,"variable", values_to="value") %>%
  ggplot(aes(value)) +
...
dat_pred_agg %>%
  st_drop_geometry() %>%
  mutate(demo_pctWhite_log = log(demo_pctWhite),
         demo_pctBlack_log = log(demo_pctBlack),
         demo_pctHisp_log = log(demo_pctHisp)) %>%
  pivot_longer(., cols = starts_with("demo_"), names_to = ,"variable", values_to="value") %>%
  ggplot(aes(value)) +
  geom_histogram() +
  labs(title = "NN Variable Distribution",
...
set.seed(717)
inTrain <- createDataPartition(y=dat_pred_agg$Night_visits_sqmi_log,
                               p = .60, list = FALSE)
#Is there a way to have the same data partition each time?

phl.training <- dat_pred_agg[inTrain,] 
phl.test <- dat_pred_agg[-inTrain,]  

# new_dat <- st_drop_geometry(phl.training)
# new_dat$Night_visits <- xx
# new_dat <- dplyr::select(new_dat, -phl_area)

colnames(dat_pred_agg)

#Multivariate regression
reg1 <- lm(Night_visits_sqmi_log ~ ., data = st_drop_geometry(phl.training %>% select(-corridor)) %>%
# reg1 <- lm(Night_visits ~ ., data = new_dat %>% 
             select(Night_visits_sqmi_log, 
                    phl_building_area_log,
...
reg1.cv <- 
  train(Night_visits_sqmi_log ~ ., data = st_drop_geometry(dat_pred_agg) %>% 
          dplyr::select(Night_visits_sqmi_log, 
                    log_phl_building_area,
                    log_sg_distance_home,
                    # sg_workday,
                    # sg_AM_PM,
                    # log_bars_a,
                    # log_rest_a,
...
Lastly, here we see the percent errors by corridor type. We seem to be best at predicting smaller corridors that are further from Center City.
```{r}
map_preds %>% 
...
* Improve the model accuracy by normalizing the variables.
* App development.