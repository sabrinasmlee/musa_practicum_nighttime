...
* Tuning hyper-parameters. Unsure how to interpret the findings and how to determine what values we should try.

* LOGO-CV for the date. The LOGO-CV indicates that the error terms are not the same across months, but all the error terms are higher than they are for the normal regression. Are we doing something wrong here.

* Trying to write a function that allows us to input all the scenario combinations and generate a csv. Need some help on this.
...
dat_pred_agg %>%
  st_drop_geometry() %>%
  dplyr::select(Night_visits, Night_visits_sqmi, Night_visits_sqmi_log) %>%
...
phl.test.sub <-
  phl.test.sub %>%
  st_drop_geometry() %>%
  dplyr::mutate(Regression = "Linear Regression Removing Outliers",
...
### Evaluating errors across time, space and type

This analysis will help determine which has the largest source of undesired variance and how we should hypertune the model's parameters.
...
```{r}
cv_preds_rf <- reg1.cv.rf$pred

map_preds <- dat_pred_agg %>% 
  rowid_to_column(var = "rowIndex") %>% 
  left_join(cv_preds_rf, by = "rowIndex") 
...
test <- map_preds %>%
  ...
  dplyr::mutate(MAE = round(abs(exp(pred) - Night_visits_sqmi),2),
                MAPE = round((MAE / Night_visits_sqmi)*100,2)) %>%
  st_drop_geometry() %>%
  dplyr::select(date_range_start, MAE, MAPE, Night_visits_sqmi) %>%
  mutate(date_range_start = str_remove_all(date_range_start, pattern = "T05:00:00Z"),
         date_range_start = str_remove_all(date_range_start, pattern = "T04:00:00Z"))
...
#   select(month, Night_visits_sqmi, MAPE) %>%
#   ggplot() +
#   geom_col(aes(x = month, y = MAPE)) +
#   geom_line(aes(x = month, y = Night_visits_sqmi), size = 1.5, color="red", group = 1)+
#   scale_y_continuous(
#     name = "Mean Average Percent Error",
#     sec.axis = sec_axis( trans=~.*1/1000, name="Night Visits")) +
#   plotTheme()
...
map_preds %>%
  ...
  dplyr::mutate(MAE = round(abs(exp(pred) - Night_visits_sqmi),2),
                MAPE = round((MAE / Night_visits_sqmi)*100,2)) %>%
  st_drop_geometry() %>%
  dplyr::select(corridor, MAE, MAPE) %>%
  kable(caption = "Errors by Space") %>%
  kable_styling()

#Comparing errors corridor type
map_preds %>%
  group_by(corr_type) %>% 
  summarise(Night_visits_sqmi = mean(Night_visits_sqmi),
            pred = mean(pred)) %>% 
...
Per the above analysis, there is the greatest variation in the space split so we are going to tune based on that feature.
...
#Splitting by date - does that make sense?
...
#Splitting by date - does that make sense?
# cv_splits_space <- group_vfold_cv(phl.training.rf%>% 
#                       dplyr::select(corridor, reg.vars) %>% 
#                       st_drop_geometry(),  
#                       strata = "Night_visits_sqmi_log", 
#                       group = "corridor")

#Splitting by date - does that make sense?
# cv_splits_type <- group_vfold_cv(phl.training.rf%>% 
#                       dplyr::select(reg.vars) %>% 
#                       st_drop_geometry(),  
#                       strata = "Night_visits_sqmi_log", 
#                       group = "corr_type")
...
#Question for Matt - how do I know the right values to try here?
...
#Space
# rf_tuned_space <- rf_wf %>%
#   tune::tune_grid(.,
#                   resamples = cv_splits_space,
#                   grid      = rf_grid,
#                   control   = control,
#                   metrics   = metric_set(rmse, rsq))
# 
...
# rf_tuned_type <- rf_wf %>%
#   tune::tune_grid(.,
#                   resamples = cv_splits_type,
#                   grid      = rf_grid,
#                   control   = control,
#                   metrics   = metric_set(rmse, rsq))

show_best(rf_tuned_time, metric = "rmse", n = 15)
# show_best(rf_tuned_space, metric = "rmse", n = 15)
# show_best(rf_tuned_type, metric = "rmse", n = 15)
...
### Reevaluating error terms after tuning hyperparameters

```{r}
rf1_predict.train_fit <- predict(rf_val_fit_time$.workflow, 
                                 new_data = st_drop_geometry(phl.training.rf_fit))
rf1_predict.train_fit <- exp(rf1_predict.train_fit %>% 
                               as.data.frame())

rf1_predict.test_fit <- predict(rf_val_fit_time$.workflow, 
                                new_data = st_drop_geometry(phl.test.rf_fit))
...
phl.test.rf_fit <-
  phl.test.rf_fit %>%
  # st_drop_geometry() %>%
  mutate(Regression = "RF Regression with Tuned Hyperparameters",
         # Visits.Predict = exp(predict(reg1.sub, phl.test.sub)),
         Visits.Predict = as.vector(rf1_predict.test_fit),
         Visits.Error = Visits.Predict - Night_visits_sqmi,
         Visits.AbsError = abs(Visits.Predict - Night_visits_sqmi),
         Visits.APE = (abs(Visits.Predict - Night_visits_sqmi)) / Visits.Predict)


rbind(phl.test.rf, st_drop_geometry(phl.test.rf_fit)) %>%
  group_by(Regression) %>%
  dplyr::summarize(MAE = mean(Visits.AbsError, na.rm = T),
                   MAPE = mean(Visits.AbsError, na.rm = T) / mean(Night_visits_sqmi, na.rm = T)) %>% 
  kable(caption = "MAE and MAPE for Test Set Data") %>% kable_styling()
...
```{r}
cv_preds <- rf_val_fit_time$.predictions
...
dat_pred_cv <-
  dat_pred_agg %>%
  dplyr::select(date_range_start, Night_visits, Night_visits_sqmi, phl_area_sqmi, reg.vars) %>%
  dplyr::mutate(uniqueID = rownames(.),
                cvID = sample(round(nrow(dat_pred_agg) / 24), size=nrow(dat_pred_agg), replace = TRUE))
...
crossValidate <- function(dataset, id, dependentVariable, indVariables) {
  ...
  for (i in cvID_list) {
    ...
    regression <-
      glm(Night_visits_sqmi_log ~ ., family = "poisson", 
          data = fold.train %>% 
...
#   dataset = dat_pred_cv,
#   id = "cvID",
#   dependentVariable = "Night_visits_sqmi_log",
#   indVariables = reg.vars) %>%
#     dplyr::select(cvID = cvID, Night_visits_sqmi_log, Prediction, geometry)
...
reg.rf.date.CV <-
  cbind(reg.rf.date.CV, dat_pred_cv %>% dplyr::select(Night_visits, Night_visits_sqmi, phl_area_sqmi)) %>%
  dplyr::select(date_range_start, 
                ...
                Prediction, 
                geometry) %>%
  dplyr::mutate(Prediction = exp(Prediction),
                Prediction = Prediction * phl_area_sqmi,
                Error = Prediction - Night_visits,
                Abs.Error = abs(Error),
                MAPE = abs(Night_visits - Prediction) / Night_visits)

reg.rf.date.CV %>%
  st_drop_geometry() %>%
  dplyr::mutate(Month = date_range_start) %>%
  group_by(Month) %>%
  summarize(MAE = mean(Abs.Error, na.rm = TRUE),
...
Scenario_preds <-
  ...
  mutate(PctChange.1 = (Preds.1 - Night_visits)/Preds.1,
         ...
         PctChange.4 = (Preds.4 - Night_visits)/Preds.4) %>%
  group_by(corridor) %>%
...
#Questions:
#Finalize the different scenarios that we want to show.
#Does this format work for the app?
#What do we do with predictions that don't make any sense? How do we measure this?
#Currently this predicts for each month. Do we want this? Do we want to aggregate up so there's only one prediction per corridor?
```

### Attempting to write a function!

```{r}
# ScenarioGenerator <- function(dataset, rest_factor) {
#   
#   allCombos <- expand.grid(restaurants = rest_factor)
#   
#   allCombos <- tibble::rowid_to_column(allCombos, "ID")
#     
#   allPredictions <- data.frame()
#   comboList <- allCombos$ID
#   
#   for (i in comboList) {
#    
#     thisScenario <- 
#       dataset %>%
#       dplyr::mutate(restaurants = restaurants * rest_factor,
#                     count_rest_a = restaurants/phl_area_sqmi,
#                     count_rest_a_log = log(count_rest_a))
#     
#     regression <-
#       ranger(Night_visits_sqmi_log ~ .,
#              data = dataset %>% dplyr::select(reg.vars))
#     
#     thisPrediction <- predict(regression, data = thisScenario)
#     thisPrediction <- exp(thisPrediction$predictions) 
#     
...
#                     count_rest_a_log = log(count_rest_a))
#     
#     thisPrediction <- predict(rf1, data = thisScenario)
#     thisPrediction <- exp(thisScenario$predictions)
#     
#     allPredictions <- rbind(allPredictions, thisPrediction)
#      
#   }
