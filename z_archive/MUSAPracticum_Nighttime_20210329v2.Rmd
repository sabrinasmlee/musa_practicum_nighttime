...
ggplot() +
  geom_sf(data = phl_boundary) +
  geom_sf(data = Temple.sf, color = "blue") +
  geom_sf(data = UPenn.sf, color = "red")
...
```{r OSM}
xmin = st_bbox(PHL_boundary_unproj)[[1]]
ymin = st_bbox(PHL_boundary_unproj)[[2]]
xmax = st_bbox(PHL_boundary_unproj)[[3]]
ymax = st_bbox(PHL_boundary_unproj)[[4]]
...
dat_pred_agg <- 
  ...
  summarize(Night_visits = sum(Hrs19_0),
            ...
            phl_rest_sqft = mean(restaurant_sqft > 0),
            phl_rest_sqft_log = log(phl_rest_sqft),
            phl_bar_sqft = mean(bars_sqft > 0),
...
```{r model building}
#Removing outliers
dat_pred_agg <- dat_pred_agg %>% filter(Night_visits_sqmi < 900000)

# summary(dat_pred_agg$Night_visits_sqmi)
# boxplot(dat_pred_agg$Night_visits_sqmi)
# hist(dat_pred_agg$Night_visits_sqmi)
# 
# summary(dat_pred_agg$Night_visits_sqmi_log)
# boxplot(dat_pred_agg$Night_visits_sqmi_log)
...
```{r model building}
#Removing outliers
# dat_pred_agg <- dat_pred_agg %>% filter(Night_visits_sqmi < 600000)
# 
# summary(dat_pred_agg$Night_visits_sqmi)
# boxplot(dat_pred_agg$Night_visits_sqmi)
# hist(dat_pred_agg$Night_visits_sqmi)
...
# 
# colnames(dat_pred_agg)
# unique(phl.training$corr_type)

#Multivariate regression
reg_nhoodsub <- 
  lm(Night_visits_sqmi_log ~ ., 
     data = st_drop_geometry(phl.training %>%
                               filter(corr_type == "1. Neighborhood Subcenter")) %>%
             select(Night_visits_sqmi_log, 
                    phl_building_size_log,
                    phl_UPenn,
                    phl_Temple,
...
reg_nhoodcent <- 
  lm(Night_visits_sqmi_log ~ ., 
     data = st_drop_geometry(phl.training %>%
                               filter(corr_type == "2. Neighborhood Center")) %>%
             select(Night_visits_sqmi_log, 
                    phl_building_size_log,
                    phl_UPenn,
                    phl_Temple,
...
reg_commcent <- 
  lm(Night_visits_sqmi_log ~ ., 
     data = st_drop_geometry(phl.training %>%
                               filter(corr_type == "3. Community Center")) %>%
             select(Night_visits_sqmi_log, 
                    phl_building_size_log,
                    phl_UPenn,
                    phl_Temple,
...
reg_regcenter <- 
  lm(Night_visits_sqmi_log ~ ., 
     data = st_drop_geometry(phl.training %>%
                               filter(corr_type == "4. Regional Center")) %>%
             select(Night_visits_sqmi_log, 
                    phl_building_size_log,
                    phl_UPenn,
                    phl_Temple,
...
reg_supreg <- 
  lm(Night_visits_sqmi_log ~ ., 
     data = st_drop_geometry(phl.training %>%
                               filter(corr_type == "5. Superregional Center")) %>%
             select(Night_visits_sqmi_log, 
                    phl_building_size_log,
                    phl_UPenn,
                    phl_Temple,
...
reg_specialty <- 
  lm(Night_visits_sqmi_log ~ ., 
     data = st_drop_geometry(phl.training %>%
                               filter(corr_type == "6. Speciality Center")) %>%
             select(Night_visits_sqmi_log, 
                    phl_building_size_log,
                    phl_UPenn,
                    phl_Temple,
...
stargazer(
  reg1, reg_nhoodsub, reg_nhoodcent, reg_commcent, reg_regcenter, reg_supreg, reg_specialty,
  type = "text",
  title = "Linear Model Summary Table",
  dep.var.caption = " ",
  dep.var.labels = "Model Features")
...
reg1.cv <- 
  train(Night_visits_sqmi_log ~ ., data = st_drop_geometry(dat_pred_agg %>% 
         filter(Night_visits_sqmi < 6659235)) %>% 
