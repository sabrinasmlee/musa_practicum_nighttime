...
# 0. Weekly Updates 4/6/2021

This week, we have continued to refine the modelling process through feature engineering and manipulating the dependent variable. Here are the major updates on our model building since last week.

* **Dependent variable**: Last week our depedent variable was aggregated across all months. After talking with Michael on Friday 4/2, we have decided to disaggregate our dependent variable by month. This means that each corridor has 12 observations. This increases the size of our dataset from about 270 to over 3000. We hope will allow us to support more model features.

* **Feature engineering**: We have continued to build and manipulate features. At this point, most of the features in our latest model are statistically significant. There are a handful that we have chosen to retain (e.g. normalized bar count) because they are important to our scenario building. We have also used the log transformation for a number of our features to make the more useful in the prediction context. We still think that the model could benefit from increased features that capture the Philadelphia built environment and the hours that an establishment remains open in the evenings.

* **Adjusted R-Squre**: We are still dealing with a relatively low Adjusted Rsquared value. 

* **Removing Outliers**: We have begun experimenting with removing outliers and have developed a workflow to compare it to our full dataset. So far, removing outliers based on normalized trip counts does not seem to be making a noticeable difference in the model's accuracy or generalizability. Should we be looking to exclude based on other variables (e.g. corridor type)?

We plan to meet with Matt on the Monday evening (4/5) before class to further discuss our approach to the model.

This week, we also added our COVID-19 graphics into our Exploratory Data Analysis section below.
...
ggplot() +
  ...
  geom_sf(data = CenterCity.sf, color = "blue") +
  geom_sf(data = CityHall.sf, color = "red")
...
We are also pulling parking data from Open Street Map (OSM). We decided to use OSM instead of the Safegraph dataset, because this shows surface parking lots in addition to garages, which are largely clustered in Center City and likely collinear with our other features.
```{r OSM}
# xmin = st_bbox(PHL_boundary_unproj %>% st_as_sf())[[1]]
# ymin = st_bbox(PHL_boundary_unproj %>% st_as_sf())[[2]]
# xmax = st_bbox(PHL_boundary_unproj %>% st_as_sf())[[3]]
# ymax = st_bbox(PHL_boundary_unproj %>% st_as_sf())[[4]]
# 
...
# ggplot() +
#   geom_sf(data = phl_boundary) +
#   geom_sf(data = parking.sf, color = "red") +
#   mapTheme()
```
...
# unique(dat_pred_temp$top_category)
dat_pred <-
  dat_pred_temp %>%
  mutate(total = 1,
         ...
         restaurant_sqft = ifelse(top_category == "Restaurants and Other Eating Places", bld_area_sqft, NA),
         bars_sqft = ifelse(top_category == "Drinking Places (Alcoholic Beverages)", bld_area_sqft, NA))
...
dat_pred_agg <- 
  ...
  summarize(Night_visits = sum(Hrs19_0),
            ...
            # phl_rest_sqft = mean(na.omit(restaurant_sqft)),
            # phl_rest_sqft_log = log(phl_rest_sqft),
            # phl_bar_sqft = mean(na.omit(bars_sqft)),
...
dat_pred_agg %>%
  st_drop_geometry() %>%
  pivot_longer(., cols = c(ends_with("a_log")), names_to = "variable", values_to="value") %>%
  ggplot(aes(value, Night_visits_sqmi_log)) +
  geom_point(size = .5) +
  geom_smooth(method = "lm", se=F, colour = "#FA7800") +
...
# reg1 <- lm(Night_visits ~ ., data = new_dat %>% 
...
In an effort to improve our Adjust R Square value, the measure indicating how well our model fits the data, we are trying to remove outliers in the data.  The following box plot shows the the distribution of the dependent variable (the log of nighttime visits normalized by corridor area) and indicates where there are outliers. **Question should we be removing outliers along another metric?**
```{r removing outliers}
dat_pred_agg %>%
  select(Night_visits_sqmi, corr_type) %>%
  ggplot(., aes(x = Night_visits_sqmi, y = corr_type)) +
  geom_boxplot() +
  coord_flip() +
  labs(title = "Outliers by Corridor Type: Normalized Night Visits",
       subtitle = "Figure X.X") +
  plotTheme()

dat_pred_agg %>%
  select(Night_visits_sqmi_log, corr_type) %>%
  ggplot(., aes(x = Night_visits_sqmi_log, y = corr_type)) +
  geom_boxplot() +
  coord_flip() +
  labs(title = "Outliers by Corridor Type: Log of Normalized Night Visits",
       subtitle = "Figure X.X") +
  plotTheme()
```

Next we code a second regression using the same features but with a subset of the data. The below output shows a comparison of the two regressions.
```{r}
#Removing outliers
dat_pred_agg_sub <- dat_pred_agg %>% filter(Night_visits_sqmi < 600000)

#Setting up test and training datasets
set.seed(414)
inTrain <- createDataPartition(y=dat_pred_agg_sub$Night_visits_sqmi_log, p = .60, list = FALSE)

phl.training.sub <- dat_pred_agg_sub[inTrain,]
phl.test.sub <- dat_pred_agg_sub[-inTrain,]
...
*Question*: Is the Night_visits_sqmi the proper variable to compare to the exponentiated prediction?
```{r}
phl.test <-
  phl.test %>%
  st_drop_geometry() %>%
...
ErrorTable <- 
  rbind(phl.test %>%
        dplyr::summarize(Regression = "Baseline Regression",
                   ...
                   MAPE = mean(Visits.AbsError, na.rm = T) / mean(Night_visits_sqmi, na.rm = T)),
      phl.test.sub %>%
        dplyr::summarize(Regression = "Subset Regression",
                   MAE = mean(Visits.AbsError, na.rm = T), 
                   MAPE = mean(Visits.AbsError, na.rm = T) / mean(Night_visits_sqmi, na.rm = T)))
...
This next chunk of code generates the above scatterplot for both the full dataset and the subset. The two charts on the left show the full dataset and the two charts on the right show the subset. Notably the prediction for the subsetted training data is doing a fairly good job aligning with the acutal values.
```{r}
...
rmse.train.sub <- caret::MAE(exp(predict(reg1.sub)), phl.training.sub$Night_visits_sqmi)
rmse.test.sub <- caret::MAE(reg1.sub_predict, phl.test.sub$Night_visits_sqmi)
...
preds.all <- rbind(preds %>% mutate(Regression = "Baseline"), 
                   preds.sub %>% mutate(Regression = "Subset"))

ggplot(preds.all, aes(x = pred, y = actual, color = source)) +
  geom_point() +
  geom_smooth(method = "lm", color = "green") +
  geom_abline(color = "orange") +
  theme_bw() +
  coord_equal() +
  facet_wrap(~Regression+source, ncol = 4) +
  labs(title = "Comparing predictions to actual values, Full Dataset & Subset",
       x = "Predicted Value",
       y = "Actual Value",
       subtitle = "Figure X.X") +
  theme(
...
reg1.cv <- 
  train(Night_visits_sqmi_log ~ ., data = st_drop_geometry(dat_pred_agg) %>% 
          dplyr::select(Night_visits_sqmi_log, 
                    phl_building_size_log,
                    # phl_UPenn,
                    phl_Temple,
                    # phl_CityHall,
...
We generate the same histogram with the subset data. The errors across the two regressions are farily uniform.
```{r}
reg1.sub.cv <- 
  train(Night_visits_sqmi_log ~ ., data = st_drop_geometry(dat_pred_agg_sub) %>% 
          dplyr::select(Night_visits_sqmi_log, 
                    phl_building_size_log,
                    # phl_UPenn,
                    phl_Temple,
...
Lastly, here we see the percent errors by corridor type. We seem to be best at predicting smaller corridors that are further from Center City. We have only produced this graphic for the full dataset.
```{r}
map_preds %>% 
